  ...
  const columns: Column<any>[] = [
    {
      header: (
        <input
          ...
          className="w-4 h-4 rounded text-sky-600 border-slate-700/80 cursor-pointer"
        />
      ),
      accessor: (d) => (
        <input
    ...
    },
    {
      ...
      accessor: (d) => (
        <div className="flex items-center space-x-3">
          <Radio className={`w-4 h-4 ${d.status === 'online' ? 'text-emerald-400 animate-pulse' : 'text-slate-400'}`} />
          <div>
            <p className="font-semibold text-white font-mono text-sm">{d.serialNumber}</p>
            <p className="text-xs text-slate-400">{d.manufacturer} {d.modelName || 'GPON ONT'}</p>
          </div>
        </div>
      ),
    },
    {
      ...
      accessor: (d) => (
        <div>
          ...
          <div className="mt-1">
            {d.ipAddress ? (
              <p className="text-xs font-mono text-slate-400">{d.ipAddress}</p>
            ) : (
    ...
    },
    {
      ...
      accessor: (d) => (
        <div>
          {d.customerId ? (
            <div>
              <p className="text-xs font-semibold text-sky-300">{d.customerId?.fullName}</p>
              <p className="text-[11px] font-mono text-slate-400">{d.customerId?.phone || d.customerId?.accountNumber}</p>
            </div>
          ) : (
            ...
            <div className="flex items-center space-x-1.5">
              ...
              {elapsedSec !== null && (
                <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded ${
                  ...
                  'bg-slate-100 text-slate-600 border border-slate-800'
                }`}>
                  {elapsedLabel}
                </span>
              )}
            </div>
            <p className="text-[11px] text-slate-400 font-mono mt-1 flex items-center space-x-1">
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-[#0E172A] p-4 border border-slate-700/80 rounded-2xl shadow-xs">
          <div className="p-3 bg-[#060913] rounded-xl border border-slate-800">
            <span className="text-[11px] text-slate-400 block font-semibold">Total ONTs in Fleet</span>
            <span className="text-xl font-mono font-bold text-white">{devices.length}</span>
          </div>
          <div className="p-3 bg-emerald-500/15 rounded-xl border border-emerald-500/30">
            <span className="text-[11px] text-emerald-400 block font-semibold">Online Fleet</span>
            <span className="text-xl font-mono font-bold text-emerald-400">
              {devices.filter((d) => d.status === 'online').length}
            </span>
          </div>
          <div className="p-3 bg-rose-500/15 rounded-xl border border-rose-500/30">
            <span className="text-[11px] text-rose-400 block font-semibold">Offline Fleet</span>
            <span className="text-xl font-mono font-bold text-rose-400">
              {devices.filter((d) => d.status !== 'online').length}
            </span>
          </div>
          <div className="p-3 bg-sky-500/15 rounded-xl border border-sky-500/30 flex items-center justify-between">
            <div>
              <span className="text-[11px] text-sky-400 block font-semibold">Selected</span>
              <span className="text-xl font-mono font-bold text-sky-400">{selectedDeviceIds.length}</span>
            </div>
        ...
        {/* Filters */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-[#0E172A] p-4 border border-slate-700/80 rounded-xl">
          <form
            onSubmit={(e) => {
              e.preventDefault();
              fetchDevices();
          ...
          <div className="space-y-5 max-h-[75vh] overflow-y-auto pr-1">
            {inspectLoading && (
              <div className="flex items-center justify-center py-12 text-slate-400 text-sm">
                <RefreshCw className="w-5 h-5 mr-2 animate-spin text-sky-400" />
            ...
            {!inspectLoading && inspectData && (
              <>
                ...
                <div className="flex items-center justify-between p-3 bg-[#0E172A] rounded-xl border border-slate-800">
                  <div className="flex items-center space-x-2">
                    <Badge variant={inspectData.hardware?.protocol === 'TR-369' ? 'purple' : 'info'}>
                      {inspectData.hardware?.protocol || 'TR-069'}
                    </Badge>
                    <span className="text-xs text-slate-400">
                      Last Updated: <span className="text-slate-100 font-mono">{inspectData.telemetry?.lastUpdated ? new Date(inspectData.telemetry.lastUpdated).toLocaleTimeString() : 'Just now'}</span>
                    </span>
                  </div>
                  <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                    ...
                    <div className="p-3 bg-[#060913] rounded-xl border border-slate-800">
                      <span className="text-[10px] text-slate-400 block font-semibold uppercase">Manufacturer</span>
                      <span className="text-xs font-semibold text-slate-100 mt-1 block">
                        {inspectData.hardware?.manufacturer ?? <span className="text-slate-400 italic">Not available</span>}
                      </span>
                    </div>
                    <div className="p-3 bg-[#060913] rounded-xl border border-slate-800">
                      <span className="text-[10px] text-slate-400 block font-semibold uppercase">MAC Address</span>
                      <span className="text-xs font-mono text-slate-100 mt-1 block">
                        {inspectData.hardware?.macAddress ?? <span className="text-slate-400 italic">Not available</span>}
                      </span>
                    </div>
                    <div className="p-3 bg-[#060913] rounded-xl border border-slate-800">
                      <span className="text-[10px] text-slate-400 block font-semibold uppercase">Firmware / Software</span>
                      <span className="text-xs font-mono text-slate-100 mt-1 block">
                        {inspectData.hardware?.firmwareVersion ?? <span className="text-slate-400 italic">Not available</span>}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-[#060913] rounded-xl border border-slate-800 space-y-3">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                    <div className="flex items-center space-x-2">
                      <Signal className="w-4 h-4 text-emerald-400" />
                      <h4 className="text-xs font-bold text-white uppercase tracking-wider">2. Live Optical Telemetry</h4>
                    </div>
                    <div className="flex items-center space-x-2">
                      <span className="text-[10px] text-slate-400">
                        Source: <span className="text-sky-400 font-mono font-semibold">{inspectData.telemetry?.source}</span>
                      </span>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1">
                    {[
                      { label: 'RX Optical Power', value: inspectData.telemetry?.rxPowerDbm, unit: 'dBm', color: 'text-emerald-400' },
                      { label: 'TX Optical Power', value: inspectData.telemetry?.txPowerDbm, unit: 'dBm', color: 'text-sky-400' },
                      ...
                      <div key={label} className="p-3 bg-[#0E172A] rounded-lg border border-slate-800">
                  ...
                  <div className="grid grid-cols-3 gap-3 pt-1">
                    <div className="p-2.5 bg-[#0E172A] rounded-lg border border-slate-800">
                      <span className="text-[10px] text-slate-400 block">Temperature:</span>
                      {inspectData.telemetry?.temperatureC != null ? (
                        <span className="text-sm font-mono font-bold text-slate-100">{inspectData.telemetry.temperatureC} °C</span>
                      ) : (
                        <span className="text-xs text-slate-400 italic">Not available</span>
                      )}
                    </div>
                    <div className="p-2.5 bg-[#0E172A] rounded-lg border border-slate-800">
                      <span className="text-[10px] text-slate-400 block">LOS Status:</span>
                      <span className={`text-xs font-mono font-bold ${inspectData.telemetry?.losStatus === 'NORMAL' ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {inspectData.telemetry?.losStatus || 'NORMAL'}
                      </span>
                    </div>
                    <div className="p-2.5 bg-[#0E172A] rounded-lg border border-slate-800">
                      <span className="text-[10px] text-slate-400 block">Optical Alarm:</span>
                ...
                <div className="p-4 bg-[#060913] rounded-xl border border-slate-800 space-y-2">
                  <div className="flex items-center space-x-2 mb-1">
                    ...
                    <h4 className="text-xs font-bold text-white uppercase tracking-wider">3. System Resources & Health</h4>
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    <div className="p-2.5 bg-[#0E172A] rounded-lg border border-slate-800">
                      <span className="text-[10px] text-slate-400 block">CPU Utilization:</span>
                      {inspectData.system?.cpuUsagePercent != null ? (
                        <span className="text-sm font-mono font-bold text-sky-400">{inspectData.system.cpuUsagePercent}%</span>
                      ) : (
                        <span className="text-xs text-slate-400 italic">Not available</span>
                      )}
                    </div>
                    <div className="p-2.5 bg-[#0E172A] rounded-lg border border-slate-800">
                      <span className="text-[10px] text-slate-400 block">Memory (RAM) Usage:</span>
                  ...
                  <div className="p-4 bg-[#060913] rounded-xl border border-slate-800 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <h4 className="text-xs font-bold text-white uppercase tracking-wider">4. Wi-Fi 2.4 GHz Band</h4>
                      </div>
                    ...
                    <div className="space-y-2 text-xs">
                      <div className="p-2.5 bg-[#0E172A] rounded-lg border border-slate-800 flex justify-between items-center">
                        <div>
                          <span className="text-[10px] text-slate-400 block font-semibold">2.4 GHz SSID:</span>
                          <span className="font-mono text-slate-100 font-bold">{inspectData.wifi?.band24?.ssid || <span className="text-slate-400 italic">Not configured</span>}</span>
                        </div>
                        <Badge variant="info">Ch {inspectData.wifi?.band24?.channel || 6}</Badge>
                      </div>
                      <div className="p-2 bg-[#0E172A] rounded text-[11px] text-slate-400 flex justify-between">
                  ...
                  <div className="p-4 bg-[#060913] rounded-xl border border-slate-800 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <h4 className="text-xs font-bold text-white uppercase tracking-wider">5. Wi-Fi 5.0 GHz Band</h4>
                      </div>
                    ...
                    <div className="space-y-2 text-xs">
                      <div className="p-2.5 bg-[#0E172A] rounded-lg border border-slate-800 flex justify-between items-center">
                        <div>
                          <span className="text-[10px] text-slate-400 block font-semibold">5.0 GHz SSID:</span>
                          <span className="font-mono text-slate-100 font-bold">{inspectData.wifi?.band5g?.ssid || <span className="text-slate-400 italic">Not configured</span>}</span>
                        </div>
                ...
                <div className="p-4 bg-[#060913] rounded-xl border border-slate-800 space-y-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <h4 className="text-xs font-bold text-white uppercase tracking-wider">6 & 7. WAN, PPPoE & VLAN Profile</h4>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-xs">
                    <div className="p-2.5 bg-[#0E172A] rounded-lg border border-slate-800">
                      <span className="text-[10px] text-slate-400 block font-semibold">WAN IP:</span>
                      <span className="font-mono text-sky-400 font-bold block mt-0.5">
                        {inspectData.wan?.wanIp || <span className="text-slate-400 italic">Not available</span>}
                      </span>
                    </div>
                    <div className="p-2.5 bg-[#0E172A] rounded-lg border border-slate-800">
                      <span className="text-[10px] text-slate-400 block font-semibold">PPPoE Username:</span>
                      <span className="font-mono text-slate-100 font-bold block mt-0.5">
                        {inspectData.wan?.pppoeUsername || <span className="text-slate-400 italic">Not available</span>}
                      </span>
                    </div>
                    <div className="p-2.5 bg-[#0E172A] rounded-lg border border-slate-800">
                      <span className="text-[10px] text-slate-400 block font-semibold">VLAN ID:</span>
                      <span className="font-mono text-slate-100 font-bold block mt-0.5">
                        {inspectData.wan?.vlanId != null ? inspectData.wan.vlanId : <span className="text-slate-400 italic">Not available</span>}
                      </span>
                    </div>
                    <div className="p-2.5 bg-[#0E172A] rounded-lg border border-slate-800">
                      <span className="text-[10px] text-slate-400 block font-semibold">WAN Connection:</span>
                ...
                <div className="p-4 bg-[#060913] rounded-xl border border-slate-800 space-y-2">
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center space-x-2">
                      ...
                      <h4 className="text-xs font-bold text-white uppercase tracking-wider">8. Connected LAN Devices</h4>
                    </div>
                  ...
                  {inspectData.lan?.connectedClients && inspectData.lan.connectedClients.length > 0 ? (
                    <div className="space-y-1.5">
                      {inspectData.lan.connectedClients.map((cl: any, idx: number) => (
                        <div key={idx} className="p-2 bg-[#0E172A] rounded-lg border border-slate-800 flex justify-between text-xs font-mono">
                          <span className="text-slate-100">{cl.hostname || 'Client Device'} ({cl.ip})</span>
                          <span className="text-slate-400">{cl.mac} | {cl.interfaceType}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="p-3 bg-[#0E172A] rounded-lg text-center text-xs text-slate-400 italic">
                ...
                <div className="p-4 bg-[#060913] rounded-xl border border-slate-800 flex items-center justify-between">
                  {inspectData.subscriber ? (
                    <div className="space-y-1">
                      <span className="text-[10px] text-slate-400 block font-semibold uppercase">Assigned Subscriber:</span>
                      <div className="flex items-center space-x-2">
                        <UserCheck className="w-4 h-4 text-emerald-400" />
                        <span className="text-sm font-bold text-white">{inspectData.subscriber.fullName}</span>
                        <span className="text-xs text-slate-400 font-mono">({inspectData.subscriber.phoneMasked})</span>
                      </div>
                      <p className="text-xs text-slate-400">Account: <span className="font-mono text-sky-400">{inspectData.subscriber.accountNumber}</span></p>
                    </div>
                  ) : (
                    <div className="space-y-1">
                      <span className="text-[10px] text-[#B45309] block font-semibold uppercase">Unassigned Device:</span>
                      <p className="text-xs text-slate-300">This ONT is currently in the unassigned hardware pool.</p>
                    </div>
                  )}
      ...
      >
        <form onSubmit={handleAssignSubmit} className="space-y-4">
          ...
          <div className="p-3 bg-[#060913] rounded-xl border border-slate-800 space-y-3">
            <h4 className="text-xs font-bold text-sky-400 uppercase tracking-wider">PPPoE & VLAN Configuration</h4>
      ...
      >
        <div className="space-y-4">
          <div className="p-4 bg-rose-500/15 border border-rose-500/30 rounded-xl flex items-start space-x-3">
            ...
            <div>
              <p className="text-sm font-bold text-white">Permanently Remove ONT Device?</p>
              <p className="text-xs text-slate-400 mt-1">
                This will delete device <code className="font-bold text-rose-400 font-mono">{deviceToDelete?.serialNumber}</code> from your fleet and unbind it from any subscriber. Real TR-069 Inform logs and history will be cleared.
              </p>
            </div>
          </div>
      ...
      >
        <div className="space-y-4">
          <div className="p-4 bg-rose-500/15 border border-rose-500/30 rounded-xl flex items-start space-x-3">
            <AlertTriangle className="w-5 h-5 text-rose-400 shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-bold text-white">Permanently Delete {selectedDeviceIds.length} ONTs?</p>
              <p className="text-xs text-slate-400 mt-1">
