      ...
      primaryAction={
        <div className="flex items-center space-x-2">
          ...
          <Button onClick={() => navigate('/operator/ai')} variant="secondary" size="sm">
            <Bot className="w-4 h-4 mr-1 text-sky-400" />
    ...
    >
      ...
      >
        ...
        <div className="p-4 sm:p-5 bg-gradient-to-r from-white via-[#F8FAFC] to-white border border-sky-500/30 rounded-2xl shadow-lg space-y-3.5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="flex items-center space-x-2">
                  <h3 className="text-sm font-bold text-white tracking-wide">TR-069 CWMP ACS Gateway</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3 pt-1">
            {/* ACS URL Box */}
            <div className="md:col-span-7 p-3.5 rounded-xl bg-[#060913] border border-slate-800 space-y-1">
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-slate-400 uppercase font-bold block">Direct ACS Server URL (ONT Config):</span>
              ...
              <span className="text-sm font-mono text-emerald-400 font-bold truncate block select-all">{activeCwmpUrl}</span>
            </div>
            ...
            <div className="md:col-span-6 grid grid-cols-3 gap-2 text-xs">
              <div className="p-2.5 rounded-xl bg-[#060913] border border-slate-800">
                <span className="text-[10px] text-slate-400 block font-semibold">ACS Username:</span>
                <span className="font-mono text-slate-100 font-bold">admin</span>
              </div>
              <div className="p-2.5 rounded-xl bg-[#060913] border border-slate-800">
                <span className="text-[10px] text-slate-400 block font-semibold">ACS Password:</span>
                <span className="font-mono text-slate-100 font-bold">admin123</span>
              </div>
              <div className="p-2.5 rounded-xl bg-[#060913] border border-slate-800">
                <span className="text-[10px] text-slate-400 block font-semibold">Inform Interval:</span>
                <span className="font-mono text-emerald-400 font-bold">60 sec</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="bg-[#0E172A] border border-slate-700/80 rounded-2xl p-5 shadow-xs space-y-3.5">
            <div className="flex items-center justify-between border-b border-slate-800 pb-2.5">
              <div className="flex items-center space-x-2">
                <Radio className="w-4 h-4 text-emerald-400 animate-pulse" />
                <h3 className="text-sm font-bold text-white">Active Reporting ONT Fleet ({s.onlineDevices || 0})</h3>
              </div>
            ...
            {s.reportingDevices && s.reportingDevices.length > 0 ? (
              <div className="divide-y divide-[#EEF2F7]">
                {s.reportingDevices.map((d: any) => (
                  <div key={d._id} className="py-2.5 flex items-center justify-between hover:bg-[#060913] px-1 rounded-lg transition">
                    ...
                    <div className="text-right space-y-1">
                      <span className={`text-xs font-mono font-bold ${d.currentRxPowerDbm < -27 ? 'text-rose-400' : 'text-emerald-400'}`}>
          ...
          <div className="bg-[#0E172A] border border-slate-700/80 rounded-2xl p-5 shadow-xs space-y-3.5">
            <div className="flex items-center justify-between border-b border-slate-800 pb-2.5">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-sm font-bold text-white">Offline / Unreachable ONTs ({s.offlineDevices || 0})</h3>
              </div>
        ...
        <div className="p-4 bg-[#0E172A] border border-slate-700/80 rounded-2xl flex items-start space-x-3.5 shadow-xs">
          <div className="p-2.5 rounded-xl bg-sky-500/15 text-sky-400 shrink-0 border border-sky-500/30">
            <Bot className="w-5 h-5" />
          </div>
          <div className="flex-1">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-white">AI Diagnostic Agent — Real-time Correlation</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div
            ...
            className="p-5 bg-[#0E172A] border border-slate-800 hover:border-slate-700/80 rounded-xl cursor-pointer transition space-y-2"
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-slate-400 uppercase">Physical Fiber GIS</span>
              <Badge variant="info">Multi-Layer</Badge>
            </div>
            <p className="text-sm font-semibold text-slate-100">Interactive GIS Map & Route Tracing</p>
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-slate-400 uppercase">ONT Fleet Controls</span>
              <Badge variant="success">TR-069 / TR-369</Badge>
            </div>
            <p className="text-sm font-semibold text-slate-100">Capability-Aware Device Controls</p>
            <p className="text-xs text-slate-400">
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-slate-400 uppercase">Field Workforce</span>
              <Badge variant="neutral">{s.activeTechnicians || 1} Online</Badge>
            </div>
            <p className="text-sm font-semibold text-slate-100">Technician Dispatch & Evidence</p>
            <p className="text-xs text-slate-400">
      ...
      >
        <div className="space-y-4">
          <div className="flex items-center justify-between bg-[#060913] p-3 rounded-xl border border-slate-800 text-xs">
            <span className="text-slate-400">
              Total Recorded Inform Packets: <strong className="text-emerald-400 font-mono">{cwmpStats?.totalHits || 0}</strong>
            </span>
            <Badge variant="success" dot>
          ...
          {(!cwmpStats?.recentHits || cwmpStats.recentHits.length === 0) ? (
            <div className="p-8 text-center bg-[#060913] rounded-xl border border-slate-800">
              <Radio className="w-8 h-8 text-slate-400 mx-auto mb-2 animate-pulse" />
              <p className="text-sm font-semibold text-slate-300">Listening for Inbound Router Traffic...</p>
              <p className="text-xs text-slate-400 mt-1">
                Configure your ONT with ACS URL <code className="text-sky-400 font-mono">{activeCwmpUrl}</code> and it will appear here automatically.
              </p>
            </div>
          ) : (
            <div className="max-h-96 overflow-y-auto space-y-2">
              {cwmpStats.recentHits.map((hit: any, i: number) => (
                ...
                >
                  <div className="space-y-0.5">
                    <div className="flex items-center space-x-2">
                      <span className="font-bold text-white font-mono">{hit.serialNumber || 'ONT Device'}</span>
                      <Badge variant={hit.status === 'PROVISIONED' ? 'success' : 'info'}>
                        {hit.status}
                      </Badge>
                    </div>
                    <p className="text-slate-400">
                      IP: <span className="font-mono text-slate-100">{hit.ip}</span> | Vendor: <span className="text-slate-100">{hit.manufacturer || 'GPON'}</span> ({hit.model || 'ONT'})
                    </p>
                  </div>
